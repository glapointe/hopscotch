...
(function (global, factory) {
  ...
  HopscotchBubble.prototype = {
      ...
      _changePosition: function _changePosition(step) {
          ...
          if (step.placement === 'left' && (boundingRect.left - bubbleBoundingWidth < 0 || boundingRect.top + bubbleBoundingHeight > window.innerHeight || boundingRect.bottom < 0)) {
              ...
              if (boundingRect.right + bubbleBoundingWidth < window.innerWidth && boundingRect.bottom + bubbleBoundingHeight < window.innerHeight) {
                  placement = 'right';
              } else if (boundingRect.top - bubbleBoundingHeight > 0 && boundingRect.right + bubbleBoundingWidth < window.innerWidth || window.innerHeight - boundingRect.top < 0) {
                  placement = 'top';
              } else if (boundingRect.bottom + bubbleBoundingHeight < window.innerHeight && boundingRect.right + bubbleBoundingWidth < window.innerWidth || boundingRect.bottom < 0) {
                  placement = 'bottom';
              }
          } else if (step.placement === 'right' && (boundingRect.right + bubbleBoundingWidth > window.innerWidth || boundingRect.top + bubbleBoundingHeight > window.innerHeight) || boundingRect.bottom < 0) {
              if (boundingRect.left - bubbleBoundingWidth < 0 && boundingRect.bottom + bubbleBoundingHeight < window.innerHeight) {
                  placement = 'left';
              } else if (boundingRect.top - bubbleBoundingHeight > 0 && boundingRect.right + bubbleBoundingWidth < window.innerWidth || window.innerHeight - boundingRect.top < 0) {
                  placement = 'top';
              } else if (boundingRect.bottom + bubbleBoundingHeight < window.innerHeight && boundingRect.right + bubbleBoundingWidth < window.innerWidth || boundingRect.bottom < 0) {
              ...
              if (boundingRect.bottom + bubbleBoundingHeight < window.innerHeight && boundingRect.right + bubbleBoundingWidth < window.innerWidth) {
                  placement = 'bottom';
              } else if (boundingRect.left - bubbleBoundingWidth < 0 && boundingRect.bottom + bubbleBoundingHeight < window.innerHeight) {
                  placement = 'left';
              } else if (boundingRect.right + bubbleBoundingWidth < window.innerWidth && boundingRect.bottom + bubbleBoundingHeight < window.innerHeight) {
                  placement = 'right';
              }
          } else if (step.placement === 'bottom' && boundingRect.bottom + bubbleBoundingHeight > window.innerHeight) {
              if (boundingRect.top - bubbleBoundingHeight > 0 && boundingRect.right + bubbleBoundingWidth < window.innerWidth) {
                  placement = 'top';
              } else if (boundingRect.left - bubbleBoundingWidth < 0 && boundingRect.bottom + bubbleBoundingHeight < window.innerHeight) {
                  placement = 'left';
              } else if (boundingRect.right + bubbleBoundingWidth < window.innerWidth && boundingRect.bottom + bubbleBoundingHeight < window.innerHeight) {
      ...
      parseHTMLString: function parseHTMLString(htmlString) {
          ...
          for (var i = 0; i < htmlString.length; i++) {
              ...
              if (char === "<") {
                  ...
                  var closingBracketIndex = htmlString.indexOf(">", i);
                  var tag = htmlString.slice(i + 1, closingBracketIndex);
                  ...
                  if (isClosingTag) {
                      ...
                      var tagName = tag.indexOf(" ") === -1 ? tag : tag.slice(0, tag.indexOf(" "));
                      if (tagName.toLowerCase() === 'svg') {
                          isSvg = true;
                      }
                      var newElement = isSvg ? document.createElementNS('http://www.w3.org/2000/svg', tagName) : document.createElement(tagName);
                      var attributes = tag.indexOf(" ") > 0 ? tag.substring(tag.indexOf(" ") + 1) : '';
                      // Iterate over the attributes string and convert to key-value pairs. The algorithm must satisfy the following rules and example.
                      // Attribute values could be wrapped in quotes or apostrophes or not wrapped at all.
                      // Attribute values could contain spaces and the equal sign could be wrapped by spaces.
                      // Attribute names may not have a value.
                      // Example format: id="my-id" class="my-class another-class" data-attr="value" required style="color: red;"
                      var attrName = "";
                      var attrValue = "";
                      var isName = true;
                      var isValue = false;
                      var isQuote = false;
                      for (var j = 0; j < attributes.trim().length; j++) {
                          var attrChar = attributes[j];
                          if (attrChar === "=") {
                              isName = false;
                              isValue = true;
                          } else if (attrChar === " " && !isQuote) {
                              if (attrName) {
      ...
      render: function render(step, idx, callback) {
          ...
          if (typeof tourSpecificRenderer === 'function') {
              try {
                  ...
                  el.appendChild(this.parseHTMLString(tourSpecificRenderer(opts)));
              }
              ...
              try {
                  ...
                  el.appendChild(this.parseHTMLString(winHopscotch.templates[tourSpecificRenderer](opts)));
              }
              ...
              try {
                  ...
                  el.appendChild(this.parseHTMLString(customRenderer(opts)));
              }
          } else {
              ...
              try {
                  ...
                  el.appendChild(this.parseHTMLString(winHopscotch.templates[templateToUse](opts)));
              }
          }
  ...
  Hopscotch = function Hopscotch(initOptions) {
      ...
      targetClickNextFn = function targetClickNextFn() {
          ...
          var el = this;
          if (_typeof(el.stepNum) === undefinedStr || el.stepNum === self.getCurrStepNum()) {
              return;
          }
          self.showStep(el.stepNum);
      },
          _addClickOrFocusToShowEvent = function _addClickOrFocusToShowEvent() {
          for (var i = 0; i < currTour.steps.length; i++) {
              ...
              if (!step.showOnTargetClickFocus) {
                  continue;
              }
          ...
          _removeClickOrFocusToShowEvent = function _removeClickOrFocusToShowEvent() {
          for (var i = 0; i < currTour.steps.length; i++) {
              var step = currTour.steps[i];
              // If were moving to the next step when a target is clicked then we don't want to show the current step when that same target is clicked.
              if (step.nextOnTargetClick) {
